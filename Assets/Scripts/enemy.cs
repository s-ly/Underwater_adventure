...
public class enemy : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.CompareTag("roket") || other.gameObject.CompareTag("player"))
        {
            Debug.Log("Бум!");
            EnemyCreate(); // БУМ!   
            Transform starTransform = this.GetComponent<Transform>(); // берём Трансформ себя (для передачи в app)
            app.GetComponent<app>().StarCreate(starTransform);  // создание звёзд из app (передача текущего положения врага)
